...
add_action( 'init', 'connexfm_acf_blocks_register', 5 );
add_filter( 'block_categories_all', 'connexfm_acf_block_category' );
add_filter( 'acf/blocks/no_fields_assigned_message', 'connexfm_acf_block_no_fields_msg', 10, 2 );
 ...
 * @since 0.1.1
 */
function connexfm_acf_blocks_register() {
	$blocks = connexfm_acf_get_blocks();
	...
	foreach ( $blocks as $block ) {
		if ( file_exists( CONNEXFM_ACF_PLUGIN_BLOCKS . $block . '/block.json' ) ) {
			/**
			 ...
			 */
			register_block_type( CONNEXFM_ACF_PLUGIN_BLOCKS . $block . '/block.json' );
		}
	}
}
 ...
 * @since 0.1.1
 */
function connexfm_acf_get_blocks() {
	// Check for options.
	$blocks  = get_option( 'connexfm_acf_blocks' );
	$version = get_option( 'connexfm_acf_blocks_version' );

	if ( empty( $blocks ) || version_compare( CONNEXFM_ACF_VERSION, $version ) || ( function_exists( 'wp_get_environment_type' ) && 'production' !== wp_get_environment_type() ) ) {
		$blocks = scandir( CONNEXFM_ACF_PLUGIN_BLOCKS );
		$blocks = array_values( array_diff( $blocks, array( '..', '.', '.DS_Store' ) ) );

		// Update our options.
		update_option( 'connexfm_acf_blocks', $blocks );
		update_option( 'connexfm_acf_blocks_version', CONNEXFM_ACF_VERSION );
	}
 ...
 * @since 0.1.1
 */
function connexfm_acf_block_category( $block_categories ) {
 ...
 * @since 0.1.1
 */
function connexfm_acf_block_no_fields_msg( $message, $block_name ) {
